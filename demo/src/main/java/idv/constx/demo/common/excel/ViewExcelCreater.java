 ...
 * @author xieyalong
 */
public class ViewExcelCreater <T extends View>  {
     ...
     */
	public HSSFWorkbook create(Collection<ExcelColumnMeta> columns,Collection<T> entities, Class<T> clazz){
		this.book = new HSSFWorkbook();
		this.columns = columns;
		this.initStyle();
		this.initSource(columns,clazz);
		if(entities == null || entities.size() == 0){
			HSSFSheet sheet = this.getSheet(defaultSheetName);
	...
	private void initStyle(){
		...
		stringCellStyle.setVerticalAlignment(HSSFCellStyle.VERTICAL_CENTER);// 垂直居中
		stringCellStyle.setAlignment(HSSFCellStyle.ALIGN_LEFT);// 水平居左
	...
	private void write(T  entity,Class<T>  clazz) throws IllegalArgumentException, IllegalAccessException{
		...
		for (int i = 0; i < source.size(); i++) {
			...
			if(renderer != null){
				style = renderer.cellStyleRender(entity, value,book, style,rowIndex);
			}
	...
	private void createHead(HSSFSheet sheet){
		...
		for (ExcelColumnMeta col: columns) {
			HSSFCell cell = row.createCell(counter);// 创建一列
    ...
    private Object getFieldValue(View obj, String[] metaNames){
		...
		if(metaNames.length > 1 && value instanceof View){
			String[] sub = new String[metaNames.length - 1];
			for (int i = 0; i < sub.length; i++) {
				sub[i] = metaNames[i+1];
			}
			return getFieldValue((View)value,sub);
		}
		return value;
    }
