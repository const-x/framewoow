...
a.find("p").attr("role","presentation");d.showDialog(a,!0);setTimeout(function(){a.focus()},100)};this.addShortcuts=function(a,b){a=a.toLowerCase();ma[a]===$&&(ma[a]=[]);ma[a].push(b)};this.delShortcuts=function(a){delete ma[a]};this.uploadInit=function(a,b,c){function f(b){M(b,"string")&&(b=[b]);var c=!1,d,e=b.length,f,k,i=[],j=[];(d=g.onUpload)&&d(b);for(d=0;d<e;d++)f=b[d],f=M(f,"string")?f:f.url,k=M(b[d],"string")?'':b[d].key,"!"===f.substr(0,1)&&(c=!0,f=f.substr(1)),i.push(f),j.push(k);a.attr('key',j.join(" "));a.val(i.join(" "));c&&a.closest(".xheDialog").find("#xheSave").click()}
